...
def _is_valid_unknown_permissions(type_: typing.Literal["authenticated", "anonymous"], /) -> AfterValidator:
    return is_valid(
...
type _AnonymousPermissions = typing.Annotated[_Permissions, _is_valid_unknown_permissions("anonymous")]
type _AuthenticatedPermissions = typing.Annotated[_Permissions, _is_valid_unknown_permissions("authenticated")]
...
class _ObjectPermissions(BaseModel):
    ...
    authenticated_users: typing.Optional[_AuthenticatedPermissions] = None
    anonymous_users: typing.Optional[_AnonymousPermissions] = None


def _object_exists(value: int) -> int:
    try:
...
type _ObjectId = typing.Annotated[int, AfterValidator(_object_exists)]


class _CopyObjectPermissions(BaseModel):
    source_object_id: _ObjectId
    target_object_id: _ObjectId
...
class AuthenticatedUserObjectPermissions(Resource):
    ...
    def put(self, object_id: int) -> ResponseData:
        request_json = flask.request.get_json(force=True)
        try:
            permissions: Permissions = validate(_AuthenticatedPermissions, request_json)
        except ValidatingError as e:
            return e.response
        object_permissions.set_object_permissions_for_all_users(
            object_id=object_id,
...
class AnonymousUserObjectPermissions(Resource):
    ...
    def put(self, object_id: int) -> ResponseData:
        ...
        request_json = flask.request.get_json(force=True)
        try:
            permissions: Permissions = validate(_AnonymousPermissions, request_json)
        except ValidatingError as e:
            return e.response
        object_permissions.set_object_permissions_for_anonymous_users(
            object_id=object_id,
