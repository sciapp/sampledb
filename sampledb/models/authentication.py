...
class SAMLArtifacts(Model):
    ...
    since: Mapped[datetime] = db.Column(db.DateTime, nullable=False, default=datetime.utcnow())

    if typing.TYPE_CHECKING:
        query: typing.ClassVar[Query["SAMLArtifacts"]]

    def __init__(self, artifact: str, message: str):
        super().__init__(artifact=artifact, message=message, since=datetime.utcnow())
...
class SAMLMetadata(Model):
    ...
    def __init__(self, component_id: int, metadata_xml: str, type: SAMLMetadataType):
        super().__init__(component_id=component_id, metadata_xml=metadata_xml, since=datetime.utcnow(), type=type)
