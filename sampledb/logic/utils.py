...
def send_confirm_email(email, id, salt):
    if id == None:
        ...
        confirm_url = flask.url_for("frontend.confirm_registration", token=token, salt=salt, _external=True)
    else:
        token = generate_token([email, id], salt=salt,
                               secret_key=flask.current_app.config['SECRET_KEY'])
        confirm_url = flask.url_for("frontend.confirm_email", token=token, salt=salt, _external=True)
