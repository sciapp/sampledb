...
def _validate_object_schema(
    ...
    for workflow_index, workflow_view in enumerate(workflow_views):
        ...
        valid_filter_operators = ['and', 'or']
        default_filter_operator = 'and'
        for filter_operator_key in ('referencing_filter_operator', 'referenced_filter_operator'):
            if workflow_view.get(filter_operator_key, default_filter_operator) not in valid_filter_operators:
        ...
        if 'recursion_filters' in workflow_view:
            ...
            default_filter_operator = 'and'
            for filter_operator_key in ('referencing_filter_operator', 'referenced_filter_operator'):
                if recursion_filters.get(filter_operator_key, default_filter_operator) not in valid_filter_operators:
                    raise ValidationError(
                        f'{filter_operator_key} must be one of:' + ', '.join(map(repr, valid_filter_operators)), path)
...
def _validate_object_reference_schema(
    ...
    valid_filter_operators = ['and', 'or']
    default_filter_operator = 'and'
    if schema.get('filter_operator', default_filter_operator) not in valid_filter_operators:
        raise ValidationError('filter_operator must be one of:' + ', '.join(map(repr, valid_filter_operators)), path)
    if 'dataverse_export' in schema and not isinstance(schema['dataverse_export'], bool):
        raise ValidationError('dataverse_export must be True or False', path)
    if 'scicat_export' in schema and not isinstance(schema['scicat_export'], bool):
        raise ValidationError('scicat_export must be True or False', path)
