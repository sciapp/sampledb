...
def delete_old_artifacts() -> None:
    SAMLArtifacts.query.filter(SAMLArtifacts.since <= datetime.datetime.utcnow() - datetime.timedelta(minutes=5)).delete()
    db.session.commit()
...
def update_metadata(component: Component, updates: dict[str, typing.Any]) -> None:
    ...
    if updates:
        if metadata_idp:
            ...
            metadata_idp.since = datetime.datetime.utcnow()
        else:
        ...
        if metadata_sp:
            ...
            metadata_sp.since = datetime.datetime.utcnow()
        else:
