...
def _sanitize_keys(
    ...
    for key, original_index in keys_with_original_indices:
        ...
        if sanitized_key[0] in string.ascii_letters and sanitized_key not in sanitized_keys.values():
            sanitized_keys[original_index] = sanitized_key
        else:
            if 'property_' + sanitized_key not in sanitized_keys.values():
                sanitized_keys[original_index] = 'property_' + sanitized_key
            else:
                for i in itertools.count(2):
                    if f'property{i}_' + sanitized_key not in sanitized_keys.values():
                        sanitized_keys[original_index] = f'property{i}_' + sanitized_key
                        break
...
def _convert_property_value_trees_to_schema_and_data(flattened_property_value_tree: FlattenedPropertyValueTree) -> typing.Tuple[typing.Dict[str, typing.Any], typing.Any, str, typing.Sequence[str]]:
    ...
    if all(isinstance(first_path_element, int) for first_path_element in first_path_elements):
        items = [
...
def _convert_property_values_to_data_and_schema(
    ...
    schema, data, _, _ = _convert_property_value_trees_to_schema_and_data(flattened_property_value_tree=flattened_property_value_tree)
