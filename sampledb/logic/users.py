...
def revoke_user_links_by_fed_ids(user_id: int, component_id: int, fed_ids: typing.Sequence[int]) -> None:
    ...
    if not has_other_auth_methods:
        remaining_rows = base_query.filter(users.User.fed_id.not_in(fed_ids)).all()
        for identity in remaining_rows:
            if identity.login:
                break
        else:
...
def identity_required_for_auth(component_id: int) -> bool:
    """
    ...
    """
    return Authentication.query.filter(
