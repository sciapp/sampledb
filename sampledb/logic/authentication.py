...
def get_login_session(login_session_id: int) -> LoginSession:
    ...
    if login.expires_at <= datetime.datetime.now(datetime.timezone.utc):
    ...
    return [
        ...
        Login.query.filter(
            db.and_(
                ...
                Login.expires_at > datetime.datetime.now(datetime.timezone.utc)
            )
        ).all()
    ]
...
def delete_all_expired_login_sessions() -> None:
    (
        db.session
        .query(Login)
        .filter(Login.expires_at < db.func.now() - datetime.timedelta(days=30))
