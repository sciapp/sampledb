...
def show_object_form(
        ...
        if object.data is None or object.schema is None:
            if object.version_component_id is not None:
                ...
                if object is None or object.data is None or object.schema is None:
                    ...
                    flask.flash(_('The current version of this object contains no data because it was disabled by the permissions and the latest version was created by another database. To edit the latest version, edit the policy. Alternatively, an older version is used for editing.'), 'warning')
            else:
                return flask.abort(400)
    ...
    if solve_conflicts and object_conflict is not None and object is not None:
        ...
        flask.flash(_("Solve the conflict between the local latest version and the latest imported version from %(component_name)s. The fields highlighted in orange are conflicting between the versions.", component_name=object_conflict.component.get_name()), 'info')
