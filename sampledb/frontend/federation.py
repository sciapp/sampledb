...
from ..logic.users import create_user, get_user_aliases_for_user, create_user_alias, update_user_alias, delete_user_alias, get_user_alias, delete_user_link, \
    ...
    revoke_user_link, enable_user_link, get_user_by_federated_user, get_user, get_active_federated_identity_by_federated_user, enable_federated_identity_for_login, identity_required_for_auth, set_user_hidden
...
def component(component_id: int) -> FlaskResponseT:
    ...
    has_other_auth_methods = identity_required_for_auth(component_id)
...
def link_identity() -> FlaskResponseT:
    ...
    try:
        ...
        identity = get_federated_identity(flask_login.current_user.id, federation_user.id)
    except (errors.FederatedIdentityNotFoundError, errors.UserDoesNotExistError):
        identity = None

    try:
        if identity is None or not identity.active:
            create_sampledb_federated_identity(flask_login.current_user.id, federation_partner, fed_user_id, update_if_inactive=True, use_for_login=True)
...
def finalize_federated_login() -> FlaskResponseT:
    ...
    if create_user_form.validate_on_submit():
        ...
        flask.flash(_('An error occurred trying to link a federated user.'), 'error')
    return flask.redirect(flask.url_for('.index'))
