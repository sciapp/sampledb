...
def test_add_multiple_users(flask_server, user_session):
    ...
    r = user_session.post(flask_server.base_url + 'projects/{}'.format(project_id), {
        'add_user': 'add_user',
        'csrf_token': csrf_token,
    ...
    for user in users:
        ...
        assert any(
            notification.type == sampledb.logic.notifications.NotificationType.INVITED_TO_PROJECT and
            notification.data['project_id'] == project_id and
            notification.data['inviter_id'] == user_session.user_id
            for notification in notifications
        )
    assert '2 users were successfully invited to the project group.' in r.content.decode('utf-8')
