...
from sampledb.models import User, Action, ActionType, Object, Objects
...
def test_view_comments(flask_server, user: User, object: Object):
    assert len(sampledb.logic.comments.get_comments_for_object(object_id=object.object_id)) == 0
    ...
    sampledb.logic.comments.create_comment(object_id=object.object_id, user_id=user.id, content="Test")
    r = session.get(flask_server.base_url + 'objects/{}'.format(object.object_id))
    assert r.status_code == 200
    document = BeautifulSoup(r.content, 'html.parser')
    ...
    comment = sampledb.logic.comments.get_comments_for_object(object_id=object.object_id)[0]
    assert comment_divs[0]['id'] == 'comment-{}'.format(comment.id)


def test_post_comments(flask_server, user: User, object: Object):
    assert len(sampledb.logic.comments.get_comments_for_object(object_id=object.object_id)) == 0
    session = requests.session()
    assert session.get(flask_server.base_url + 'users/{}/autologin'.format(user.id)).status_code == 200
    r = session.get(flask_server.base_url + 'objects/{}'.format(object.object_id))
    assert r.status_code == 200
    document = BeautifulSoup(r.content, 'html.parser')
    comment_form = document.find('form', {'action': '/objects/{}/comments/'.format(object.object_id), 'method': 'post'})
    ...
    r = session.post(flask_server.base_url + 'objects/{}/comments/'.format(object.object_id), data=data)
    assert r.status_code == 200
    assert len(sampledb.logic.comments.get_comments_for_object(object_id=object.object_id)) == 1
    comment = sampledb.logic.comments.get_comments_for_object(object_id=object.object_id)[0]
