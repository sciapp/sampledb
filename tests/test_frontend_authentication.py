...
def app():
    ...
    def autologin(user_id):
        user = sampledb.authentication.models.User.query.get(user_id)
...
def user(flask_server):
    with flask_server.app.app_context():
        user = sampledb.authentication.models.User(name="Basic User", email="example@fz-juelich.de", type=sampledb.authentication.models.UserType.PERSON)
...
def test_sign_in(flask_server):
    ...
    r = session.get(flask_server.base_url + 'frontend/')
    ...
    r = session.get(flask_server.base_url + 'frontend/users/me/sign_in')
    assert r.status_code == 200
    ...
    # submit the form
    r = session.post(flask_server.base_url + 'frontend/users/me/sign_in', {
...
def test_sign_in_authenticated(flask_server, user):
    ...
    r = session.get(flask_server.base_url + 'frontend/users/me/sign_in', allow_redirects=False)
...
def test_sign_out_navbar(flask_server, user):
    ...
    assert r.status_code == 200
    document = BeautifulSoup(r.content, 'html.parser')
    sign_out_form = document.find('form', {'action': '/frontend/users/me/sign_out', 'method': 'post'})
    ...
    # submit the form
    r = session.post(flask_server.base_url + 'frontend/users/me/sign_out', {
        'csrf_token': csrf_token
    })
    assert session.get(flask_server.base_url + 'users/me/loginstatus').json() is False
...
def test_sign_out_page(flask_server, user):
    ...
    r = session.get(flask_server.base_url + 'frontend/users/me/sign_out')
    assert r.status_code == 200
    document = BeautifulSoup(r.content, 'html.parser')
    sign_out_form = document.find('form', {'action': '/frontend/users/me/sign_out', 'method': 'post'})
    assert sign_out_form is not None
...
def test_sign_out_page_unauthenticated(flask_server, user):
    ...
    r = session.get(flask_server.base_url + 'frontend/users/me/sign_out', allow_redirects=False)
    assert r.status_code == 302
    assert r.headers['Location'].startswith(flask_server.base_url + 'frontend/users/me/sign_in')
    assert session.get(flask_server.base_url + 'users/me/loginstatus').json() is False
